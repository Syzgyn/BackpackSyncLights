...
void waves(CRGB *array, uint32_t now) {
  ...
  for (int k=0; k<NUM_LEDS; k++) {                              // For each of the LED's in the strand, set a brightness based on a wave as follows:
    ...
    int thisBright = qsuba(colorIndex, beatsin8(7,0,40));                                 // qsub gives it a bit of 'black' dead space by setting sets a minimum value. If colorIndex < current value of beatsin8(), then bright = 0. Otherwise, bright = colorIndex..
...
void lava(CRGB *array, uint32_t now) {
  ...
  for (int k=0; k<NUM_LEDS; k++) {                              // For each of the LED's in the strand, set a brightness based on a wave as follows:
    ...
    array[k] = ColorFromPalette(LavaColors_p, colorIndex, thisBright, LINEARBLEND);  // Let's now add the foreground colour.
  }
